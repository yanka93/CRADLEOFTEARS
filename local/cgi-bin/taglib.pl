        ...
        foreach my $row (@$counts) {
            ...
            if ($sec & $public_mask) {
                ...
                $res->{$jid}->{$kwid}->{security_level} = 'friends'
                    if $res->{$jid}->{$kwid}->{security_level} &&
		      $res->{$jid}->{$kwid}->{security_level} ne 'public';
            } elsif ($sec) {
...
sub get_logtags {
    ...
    my @ids = 
    my $ret = LJ::Tags::get_logtagsmulti([ map { [ $u->{clusterid}, $u->{userid}, $_ ] } @$jitemid ]);
...
# args: ids
# des-ids: An arrayref of [clusterid, jid, jitemid]
...
sub get_logtagsmulti {
    ...
    # construct memcache keys for loading below
        foreach my $row (@$ids) {
            my ($cid, $jid, $jitemid) = ($row->[0], $row->[1], $row->[2]);
            $need{$cid}->{$jid}->{$jitemid} = 1;
            $jid2cid{$jid} = $cid;
            push @memkeys, [ $jid, "logtag:$jid:$jitemid" ];
        }
